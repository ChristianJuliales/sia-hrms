document.addEventListener("DOMContentLoaded", () => {
    ...
    if (loggedInUserString) {
        try {
            ...
            // Assuming loggedInUser.email or similar is available
    ...
    async function updateLeaveStatus(id, status) {
        try {
            ...
            let updateData = {
                status: status,
                // Ensure updated_at is always set for the leave_requests table
    ...
    async function fetchLeaveBalance(employeeId) {
        try {
            ...
            const { data, error } = await supabase
                .from('leave_balances')
                .select('balance') // Only select the necessary field
                .eq('employee_id', employeeId)
                .single();

            // PGRST116 means no rows found (which is fine, default to 12)
            ...
            console.error('Error fetching balance:', error);
            // Default return in case of catastrophic error
            return 12;
        }
    }
    ...
    async function updateLeaveBalance(employeeId, newBalance) {
        try {
            ...
            // 1. Check for existing balance record
            ...
            if (existing) {
                // 2. Update existing record - FIX: Added updated_at
                const { error } = await supabase
                    .from('leave_balances')
                    .update({
                        balance: newBalance,
                        updated_at: now // FIX: ADDED updated_at
                    })
                    .eq('employee_id', employeeId);

                if (error) throw error;
            } else {
                // 3. Insert new record - FIX: Added created_at and updated_at
                const { error } = await supabase
                    .from('leave_balances')
                    .insert([{
                        ...
                        created_at: now, // FIX: ADDED created_at
                        updated_at: now  // FIX: ADDED updated_at
                    }]);
    ...
    // Check if dateFilter exists before trying to access its value
    const initialFilter = dateFilter ? dateFilter.value : 'all';
    renderManagerTable(initialFilter);
    ...
    supabase
        ...
        .on('postgres_changes', { event: '*', schema: 'public', table: 'leave_requests' }, (payload) => {
            ...
            renderManagerTable(dateFilter ? dateFilter.value : 'all');
        })
        .subscribe((status) => {
